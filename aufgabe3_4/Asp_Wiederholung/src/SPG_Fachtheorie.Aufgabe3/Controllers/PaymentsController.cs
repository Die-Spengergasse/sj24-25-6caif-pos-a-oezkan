...
namespace SPG_Fachtheorie.Aufgabe3.Controllers
{
    ...
    public class PaymentsController : ControllerBase
    {
        ...
        public IActionResult AddPayment([FromBody] Commands.NewPaymentCommand cmd)
        {
        ...
        {
            if (id != cmd.Id) return Problem("Invalid Payment item ID", statusCode: 400);
            
            var paymentItem = _db.PaymentItems
                .Where(pi => pi.Id == id)
                .FirstOrDefault();

            if (paymentItem == null) return Problem("Payment Item not found", statusCode: 404);

            if ((paymentItem.LastUpdated.HasValue || cmd.LastUpdated.HasValue) && paymentItem.LastUpdated != cmd.LastUpdated) 
                return Problem("PaymentItem has been changed", statusCode: 400);
            var payment = _db.Payments.FirstOrDefault(pi => pi.Id == id);
            if(payment == null) return Problem("Invalid Payment ID", statusCode: 400);
        ...
        [HttpPatch("{id}")]
        public IActionResult updatePaymentConfirmed(int id, [FromBody] UpdateConfirmedCommand cmd)
        {
            var payment = _db.Payments.FirstOrDefault(pi => pi.Id == id);
            if (payment == null) return Problem("No Payment Found", statusCode: 404);

            if (payment.Confirmed.HasValue) return Problem("Payment already Confirmed", statusCode: 400);

            if (!cmd.Confirmed.HasValue) return Problem("No valid Date set", statusCode: 400);

            payment.Confirmed = cmd.Confirmed.Value;
